<?php
/*
Based on: Blogger Importer Version: 0.4
*/
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class Selective_Blogger_Import extends WP_Importer {

	// Shows the welcome screen and the magic auth link.
	function greet() {
		$next_url = get_option('siteurl') . '/wp-admin/index.php?import=selective-blogger&amp;noheader=true';
...
//echo '<pre>'.print_r($this,1).'</pre>';
	...
	function restart() {
		...
		wp_redirect('?import=selective-blogger');
	}
	...
	function congrats() {
		$blog = (int) $_GET['blog'];
		echo '<h1>'.__('Congratulations!', 'blogger-importer').'</h1>';
		echo '<p><a href="' . admin_url('edit.php?post_type=import') . '">' . __( 'Choose posts to import...', 'selective-importer' ) . '</a>' . '</p>';
		echo '<p>'.__('For security, click the link below to reset this importer.', 'blogger-importer').'</p>';
	}
	...
	function start() {
		...
		if ( $saved && !isset($_GET['noheader']) ) {
			...
			echo "<div class='wrap'><h2>$restart</h2><p>$message</p><form method='post' action='?import=selective-blogger&amp;noheader=true'><p class='submit' style='text-align:left;'><input type='submit' class='button' value='$submit' name='restart' /></p></form></div>";
		}
	}
...
<?php
	...
	function Selective_Blogger_Import() {
		global $importer_started;
...
$blogger_import = new Selective_Blogger_Import();

register_importer('selective-blogger', __('Selective Blogger', 'blogger-importer'), sprintf(__('Select posts to import from a Blogger blog. <a href="%s">View pending imports.</a>', 'blogger-importer'), 'edit.php?post_type=import'), array ($blogger_import, 'start'));
